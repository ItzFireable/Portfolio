<script setup lang="ts">
  import { ref } from 'vue';

  const baseUrl = import.meta.env.VITE_API_URL || import.meta.env.VITE_API_BACKUP_URL;

  let urlParams = new URLSearchParams(window.location.search);
  const uuid = ref(urlParams.get('uuid'));

  const connectUrl = ref(baseUrl + "/scrobbler/connect/tachi");
  const disconnectUrl = ref(baseUrl + "/scrobbler/disconnect?uuid=" + uuid.value);
</script>

<template>
  <div v-bind:class="'project left'">
    <img v-bind:src="'/chuni.png'" class="banner" alt="Project banner" />
    <h1 v-bind:class="'title text-left'">Kamaitachi Last.fm Sync</h1>
    <p v-bind:class="'description text-left'">Syncs all the songs possible from Kamaitachi into Last.fm, based on your plays.<br /><br />Should be automatic after initial setup, I cannot guarantee this will work forever!</p>

    <div v-if="uuid" class="box">
        <a v-if="disconnectUrl" v-bind:href="disconnectUrl" class="connect-btn">
            <i class="fa-solid fa-link"></i>
            Disconnect
        </a>
    </div>
    <div v-else class="box">
        <a v-if="connectUrl" v-bind:href="connectUrl" class="connect-btn">
            <i class="fa-solid fa-link"></i>
            Login with Kamaitachi
        </a>
    </div>
  </div>
</template>

<style scoped>
.project {
  height: auto;
  width: calc(35% - 48px);

  margin-bottom: 8px;
  margin-top: 8px;
  display: flex;

  flex-direction: column;

  border-radius: 16px;
  box-shadow:
    0 1px 1px hsl(0deg 0% 0% / 0.2),
    0 2px 2px hsl(0deg 0% 0% / 0.2),
    0 4px 4px hsl(0deg 0% 0% / 0.2),
    0 8px 8px hsl(0deg 0% 0% / 0.2),
    0 16px 16px hsl(0deg 0% 0% / 0.2);

  justify-content: center;
  align-items: center;

  position: relative;
  overflow: hidden;
  padding: 16px;
  padding-top: 0px;
  padding-inline: 24px;
}

.project>* {
  z-index: 2;
  text-shadow: 0px 0px 12px rgba(0, 0, 0, 0.8);
}

.description {
  width: 80%;
}

.text-left {
  text-align: left;
}

.text-right {
  text-align: right;
}

.left {
  align-items: flex-start;
  z-index: 2;
}

.right {
  align-items: flex-end;
  z-index: 2;
}

.banner {
  width: 100%;
  height: 100%;
  position: absolute;
  object-fit: cover;
  z-index: 1 !important;
  transform: scale(1.2);
  filter: brightness(40%) blur(12px);

  user-drag: none;
  -webkit-user-drag: none;
  user-select: none;
  -moz-user-select: none;
  -webkit-user-select: none;
  -ms-user-select: none;
}

.box {
  width: auto;
  height: auto;

  background: rgba(18, 18, 18, 0.6);
  backdrop-filter: blur(8px);

  display: flex;

  border-radius: 16px;
  box-shadow:
    0 1px 1px hsl(0deg 0% 0% / 0.2),
    0 2px 2px hsl(0deg 0% 0% / 0.2),
    0 4px 4px hsl(0deg 0% 0% / 0.2),
    0 8px 8px hsl(0deg 0% 0% / 0.2),
    0 16px 16px hsl(0deg 0% 0% / 0.2);

  justify-content: center;
  align-items: center;

  position: relative;
  overflow: hidden;

  padding: 12px;
  text-shadow: 0px 0px 12px rgba(0, 0, 0, 0.4);
  margin-left: -8px;
  margin-right: -8px;
}

.connect-btn:link {
  color: gray;
  text-decoration: none;
}

.connect-btn:visited {
  color: gray;
  text-decoration: none;
}

.connect-btn:hover {
  color: white;
  text-decoration: none;
}

.connect-btn:active {
  color: gray;
  text-decoration: none;
}

@media (max-width: 768px) {
  .project {
    width: calc(100% - 48px);
  }
}
</style>
